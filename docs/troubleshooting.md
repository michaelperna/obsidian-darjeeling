...
# if claude is not on the service user's PATH:
sudo runuser -u darjeeling -- /var/lib/darjeeling/.local/bin/claude login
```
...
### Resolution:
1. **Hostname vs. IP Literal**: Apple App Transport Security (ATS) blocks plain `http://` and `ws://` to domain names (e.g. `http://server.local:8765`). On iOS, use either:
   - A numeric IP literal: `http://100.x.y.z:8765`
   - An HTTPS endpoint: `https://workstation.tailnet.ts.net` via Tailscale Serve.
2. **Local Network Permission**: If connecting over LAN Wi-Fi, ensure Obsidian has permission to access the local network:
